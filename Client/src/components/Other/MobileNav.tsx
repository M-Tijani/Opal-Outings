  ...
  return (
    <>
      <section className="relative w-full">
        <section className="bg-primary shadow-md flex items-center justify-between h-full my-2 mx-2 py-2 px-4 rounded-md">
          <Link to={"/"}>
            <img className="w-[180px]" src={Logo} alt="" />
          </Link>
          <section className="flex items-center justify-center gap-4">
            <OutsideClickHandler
              onOutsideClick={() => setShowingProfile(false)}
            >
              ...
              {isUserLoggedIn ? (
                  ...
                  <AnimatePresence>
                    {showingProfile && (
                      <motion.section
                        ...
                        animate={{ x: 0 }}
                        exit={{ x: 200 }}
                        className="bg-white ring-1 ring-tertiary absolute duration-200 top-[90px] right-2 w-full max-w-[300px] h-[450px] flex flex-col rounded-md shadow-md"
                      >
                        <section className="w-full flex flex-col gap-2">
                          <Avatarinput
                            ...
                            email={avatarEmail}
                          />
                          <span className="w-full h-[2px] rounded-md max-w-[270px] mx-auto bg-tertiary"></span>
                          ...
                          <span className="w-full h-[2px] rounded-md max-w-[270px] mx-auto bg-tertiary mt-4"></span>
                          ...
                          <span className="absolute top-[285px] left-[40px] text-tertiary">
                            Support
                          </span>
                        </section>
                      </motion.section>
                ...
                <>
                  {showingProfile && (
                    <section
                      onClick={handleshowingprofile}
                      className="absolute inset-0 w-full h-screen bg-black opacity-30"
                    ></section>
                  )}
                  <AnimatePresence>
                    {showingProfile && (
                      <>
                        <motion.section
                          ...
                          animate={{ x: 0 }}
                          exit={{ x: 200 }}
                          className="absolute duration-200 top-[90px] right-2 w-full max-w-[300px] h-[250px] flex flex-col rounded-md bg-white shadow-md"
                        >
                          <section className="w-full flex flex-col gap-2">
                            <Avatarinput
                              name={avatarName}
                  ...
                  <>
                    <section className="">
                      <motion.div
                        ...
                        animate={{ x: 0 }}
                        exit={{ x: 200 }}
                        className="Boxholderformenu shadow-md"
                      >
